  ...
  const storeReceiptData = (receiptData) => {
    ...
    if (receiptData.balanceType === 'package') {
      if (packageIds?.length) {
        ...
        updatedPackageIds = [{ agencyUrl: activeAppSettings.agencyUrl, tokenIds: [packageDetail.tokenId] }]
      }
    }
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (type === 'erc20') receipt = await rahatService.verifyChargeForERC20(phone, otp);
